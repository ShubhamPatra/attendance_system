...
def test_allowed_file_valid():
    from app_core.utils import allowed_file
...
def test_today_str_format():
    from app_core.utils import today_str
    result = today_str()
...
def test_now_time_str_format():
    from app_core.utils import now_time_str
    result = now_time_str()
    assert len(result) == 8
...
def test_check_image_quality_passes():
    ...
    from app_core.utils import check_image_quality
