...
# Check both PyTorch and ONNX Runtime for GPU support
def _detect_device() -> str:
    """Detect available GPU device for face engine."""
    # First check PyTorch CUDA
    if torch.cuda.is_available():
        return "cuda"
    
    # Then check ONNX Runtime providers
    try:
...
class EncodingCache:
    ...
    def get_flat(self) -> tuple[np.ndarray | None, np.ndarray | None, list, list[str]]:
        ...
        The flat arrays are replaced atomically by ``_rebuild_flat`` (never
        mutated in place), so we return direct references instead of
        expensive copies.
        """
        with self._lock:
            return (
                self._flat_encodings,
                self._flat_student_idx,
                list(self._ids),
                list(self._names),
            )
    ...
    def get_stage1(self) -> tuple[np.ndarray | None, np.ndarray | None, list, list[str]]:
        ...
        Arrays are replaced atomically — direct references are safe.
        """
        with self._lock:
            return (
                self._student_prototypes,
                self._prototype_student_idx,
                list(self._ids),
                list(self._names),
            )
